...
#'
#'
CentCurv<-function(x,y,window.select="CV",method,lambdas=0,cents=c(0.03,0.25,0.5,0.75,0.97)){
  if (method=="Local constant"){localCst(x,y,window.select,cents=cents)}
  else{if (method=="Local linear"){localLin(x,y,cents=cents)}
    else{if(method=="Cubic splines"){cubicSpline(x,y,cents=cents)}
      else{if(method=="B-Splines"){bsplines(x,y,lambdas,cents=cents)}

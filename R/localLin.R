...
#' localLin(x,y)
#'
localLin<-function(x,y,bandwidth.method="CV", kernel = "gauss", cents=c(0.03,0.25,0.5,0.75,0.97), disp_window = TRUE){
  x_seq=seq(min(x),max(x),0.01)
  bandwidth<-window(x,y,bandwidth.method)
  kern = gaussK
  if(kernel == "gauss"){kern = gaussK}
  if(kernel == "epan"){kern = EpaK}
  if(kernel == "trig"){kern = TrianK}
  lcw<-locpol::locLinWeightsC(x,x_seq,bandwidth,kern)$locWeig
  m= lcw %*% y
  v= lcw %*% (y*y)-m^2
  s=sqrt(v)
